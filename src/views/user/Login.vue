<template>
    <div class="box">
        ...
        <div class="aside-tips">
            ...
            <router-link to="/register" class="getPassword">去注册</router-link>
        </div>
    </div>
</template>
<script>
        ...
        onSubmit(values) {
            ...
            axios.post(`http://localhost:3001/api/login`, values).then(res=>{
                ...
                if(res.data.err == 1){
                    ...
                    // 登录成功
                    alert("登录成功")
                    sessionStorage.setItem('mlh_token', res.data.token)
                    sessionStorage.setItem('mlh_id', res.data.data._id)
